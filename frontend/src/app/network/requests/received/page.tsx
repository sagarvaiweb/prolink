...
import { useGetReceivedConnectionRequestsQuery } from "@/redux/features/networking/networkingApi";
...
function ReceivedRequestsContent() {
  ...
  const { data, error, isError, isFetching, isLoading, refetch } =
    useGetReceivedConnectionRequestsQuery({ page, limit: PAGE_SIZE });

  useEffect(() => {
    dispatch(setActiveContext("receivedRequests"));
  }, [dispatch]);

  const requestData = data?.data;
  const isInitialLoading = isLoading && !requestData;
  ...
  if (isError && !requestData) {
    return (
      <NetworkingErrorState
        message={getNetworkingErrorMessage(error, "Unable to load connection requests.")}
        onRetry={refetch}
      />
    );
  }

  if (!requestData || requestData.pagination.totalRequests === 0) {
    return (
      <NetworkingEmptyState
  ...
  const { pagination, requests } = requestData;
    ...
    <div className="space-y-5">
      {isError && (
        <NetworkingErrorState
          message={getNetworkingErrorMessage(
            error,
            "Unable to refresh your connection requests."
          )}
          onRetry={refetch}
        />
      )}
      ...
      <div className="grid gap-3">
        {requests.map((request) => (
          <ConnectionRequestCard key={request._id} request={request} mode="received" />
        ))}
      </div>

      <nav
        aria-label="Received connection requests pagination"
...
export default function ReceivedConnectionRequestsPage() {
  return (
    <ProtectedRoute>
        ...
        <div className="mx-auto max-w-4xl">
          <header className="mb-6">
            ...
            <p className="mt-2 max-w-2xl text-sm leading-6 text-gray-600 sm:text-base">
              Review people who would like to connect with you.
            </p>
          </header>
